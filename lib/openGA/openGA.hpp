        ...
        void check_settings() {
            if (is_interactive()) {
                ...
                if (eval_solution != nullptr)
                    throw runtime_error(
                            "eval_solution is not null in interactive mode (use eval_solution_IGA instead)!");
            } else {
                if (calculate_IGA_total_fitness != nullptr)
...
//                    }
	...
	****************************************************/
        template<void (thisType::*action_function)(thisGenerationType *p_generation0, int index_from, int index_to,
                                                   unsigned int *attemps, std::atomic<bool> &active_thread)>
